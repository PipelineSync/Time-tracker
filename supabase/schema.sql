...
-- ============================================================================
-- IT SUPPORT TICKETS (same content as supabase/ticket-support.sql)
-- ----------------------------------------------------------------------------
-- The last section, because the tables reference everything above it: profiles
-- and workers for the grant, auth.users for the requester and the assignee.
-- What it adds, in one place:
--   * public.is_it_support() — the worker-only desk check. NOT has_permission:
--     that answers true for the admin for every key, and the admin is exactly
--     who must not get the queue.
--   * tickets + ticket_replies with RLS, so anyone can submit and read their
--     own ticket while only the desk sees the queue.
--   * public.it_support_holders() — who a ticket can be assigned to.
--   * notifications: the 'ticket' type + the ticket_id link.
--   * the SECURITY DEFINER fan-out triggers (a worker cannot insert
--     notifications for anyone but themselves under RLS, so the database
--     tells the desk when a ticket arrives, is replied to, or moves).
-- ============================================================================

-- ---------- 1. the capability key (re-stated; the allow-list above already
-- has it, and this keeps the two files identical) ----------
