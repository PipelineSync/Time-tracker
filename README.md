...
>
> **Shortcut for the latest changes:** **`supabase/RUN-THIS-payments-and-reports-update.sql`** is one copy-paste bundle — payment method + reference number columns, the worker payment-method columns and self-service RPC, and the widened `time_entries` policy that makes a Reports grant cover the whole team. Safe to re-run, with verification queries at the end.
>
