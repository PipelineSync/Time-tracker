...
### In-app FAQs
A **very small circle with a question mark** sits at the **upper-left of the section it explains** — on **Clock In / Out** for a worker, on the **Dashboard** for the admin (a worker granted the Dashboard gets the worker answers there, because the button follows the *account* unless a screen pins the audience). It opens a short accordion of the questions that actually come up, each answer naming the screen to use:

- **Workers** — clocking in (a client is required) and why the button can be disabled, how break time is subtracted, what to do about a forgotten clock-out, switching client mid-shift, where hours and settlements show up, how to set Cash / QR payment methods, changing a password, adding a profile picture, and what stays private.
- **Admins** — adding a worker and creating/resetting their login, granting (and revoking) access to admin screens, adding time someone forgot to log, the live "On the clock now" panel, fixing a timer left running, where clients come from, settling and paying a worker, subscriptions/bills/due dates, reports & CSV, and the Settings that decide currency and notifications.
