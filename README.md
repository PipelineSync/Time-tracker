...
- **"Claim your bonus" prank** — a gift button on the Clock In / Out screen that leans away and springs to a new random spot whenever the cursor gets within ~100px, so it can never be caught (`src/components/ClaimBonusButton.tsx`). It is decoration, not a control: no click handler, `tabIndex={-1}`, `aria-hidden`, and it never rests on top of the real clock-in/out controls. **Desktop only** (`hidden lg:block`, the same `lg` the app shell uses for its own mobile/desktop split) — on a phone every "hover" is a finger tap, so the prank would just be tap-bait. `npm run verify:claim-bonus` locks that in.
